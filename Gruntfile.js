...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    browserify: {
      ...
      unittests: {
        ...
        options: {
          external: [ 'crypto', 'buffer' , 'node-localstorage', 'node-fetch', 'openpgp', './app/test/testrunner.js', './app/test/test.js' ],
          transform: [
            ["babelify", {
              ignore: ['*.min.js'],
              presets: ["es2015", "es7"]
        ...
        files: {
          'test/lib/unittests-bundle.js': [ './test/testrunner.js' ]
        }
      }
    },
    replace: {
    ...
    mochaTest: {
      unittests: {
        ...
        src: [ 'test/testrunner.js' ] // unittests.js
      }
    },
    copy: {
      browsertest: {
  ...
  grunt.registerTask('prepare-tests', ['copy:browsertest', 'connect:test', 'saucelabs-mocha']);
